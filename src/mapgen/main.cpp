...
void callbackData(const node msg)
{
    ...
    if ((cnt % viz_interval) == 0)
    {
        ...
        cloudPublisher.publish(erasor_utils::cloud2msg(*cloudCurr)); // 实时点云
        mapPublisher.publish(erasor_utils::cloud2msg(*cloudMap));    // 实时累加地图
        pathPublisher.publish(path);                                 // 路径
    }
    cnt++;
}
...
int main(int argc, char **argv)
{
    ...
    nodeHandler.param<int>("/map/viz_interval", viz_interval, 10); // 可视化点云序列间隔
    ...
    interval = std::stoi(name_parsed[6]); // 包中点云序列间隔
