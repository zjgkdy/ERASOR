...
private:
    ...
    void check_movement(const pcl::PointCloud<pcl::PointXYZI> &cloud)
    {
        ...
        if (not_moving_object_candidates.empty())
        { // init
            for (auto it = clusters_tmp.begin(); it != clusters_tmp.end(); it++)
            {
...
public:
    ...
    /**
     * @brief 累加点云到地图云，解析位姿存储到路径容器中
     *
     * @param data 点云、位姿数据
     * @param path 路径容器
     */
    void accumPointCloud(const erasor::node &data, nav_msgs::Path &path)
    {
        ...
        pcl::transformPointCloud(cloud, *ptr_transformed, tf_lidar2origin); // lidar系 -> body系
        ...
        pcl::transformPointCloud(*ptr_transformed, *world_transformed, pose);         // body系 -> 世界系
        erasor_utils::voxelize_preserving_labels(world_transformed, cloud_curr, 0.2); // 体素化 + 近邻搜索赋值标签
     ...
     * @brief 保存原始地图云
     *
     * @param original_dir 不经处理的地图云保存目录
     * @param map_dir 体素化后的地图云保存目录
     */
    void saveNaiveMap(const std::string &original_dir, const std::string &map_dir)
    {
        ...
        pcl::io::savePCDFileASCII(original_dir, cloud_src); // 保存原始的地图云
        ...
        *ptr_map = cloud_src;
        ...
        pcl::io::savePCDFileASCII(map_dir, cloud_out); // 保存体素化后的地图云
