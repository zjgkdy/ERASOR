...
void OfflineMapUpdater::load_global_map()
{
    ...
    int failure_flag = erasor_utils::load_pcd(map_name_, map_init_); // 加载原始地图
        ...
        *map_arranged_init_ = *map_arranged_;
        if (is_large_scale_)
        {
            ...
            *map_arranged_global_ = *map_arranged_;
            ...
            std::cout << "Submap size is: " << submap_size_ << std::endl; // 子地图尺寸
        }
    }
 ...
 * @brief 提取子地图
 *
 * @param pose_x 机器人body实时x坐标
 * @param pose_y 机器人body实时y坐标
 */
...
void OfflineMapUpdater::set_submap(const pcl::PointCloud<pcl::PointXYZI> &map_global, pcl::PointCloud<pcl::PointXYZI> &submap,
                                   pcl::PointCloud<pcl::PointXYZI> &submap_complement, double x, double y, double submap_size)
{
 ...
 * @brief 将位姿信息记录在path中
 *
 * @param path 保存路径信息的变量
 * @param mode 无
 * @param node 提供header信息
 * @param body2mapprev 提供位姿信息
 */
void OfflineMapUpdater::set_path(nav_msgs::Path &path, std::string mode, const erasor::node &node, const Eigen::Matrix4f &body2mapprev)
{
