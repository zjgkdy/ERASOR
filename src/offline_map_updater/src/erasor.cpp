 ...
 */
void ERASOR::set_inputs(const pcl::PointCloud<pcl::PointXYZI> &map_voi, const pcl::PointCloud<pcl::PointXYZI> &query_voi)
{
...
void ERASOR::voi2r_pod(const pcl::PointCloud<pcl::PointXYZI> &src, R_POD &r_pod)
{
    ...
    sensor_msgs::PointCloud2 pc2_curr_init = erasor_utils::cloud2msg(curr_init, "camera_left");
 ...
 * @param src           输入点云
 * @param r_pod         未占用率描述子
 * @param complement    R_POD区域外的互补点云
 */
void ERASOR::voi2r_pod(const pcl::PointCloud<pcl::PointXYZI> &src, R_POD &r_pod, pcl::PointCloud<pcl::PointXYZI> &complement)
{
    // 更新区域的伪占用率描述子
    for (auto const &pt : src.points)
    {
    ...
    sensor_msgs::PointCloud2 pc2_map_init = erasor_utils::cloud2msg(map_init, "camera_left");
...
void ERASOR::get_outliers(pcl::PointCloud<pcl::PointXYZI> &map_rejected, pcl::PointCloud<pcl::PointXYZI> &curr_rejected)
{
...
void ERASOR::compare_vois_and_revert_ground(int frame)
{
    ...
    poly_list.header.frame_id = "camera_left";
    ...
    // For debugging
    sensor_msgs::PointCloud2 pc2_map_r = erasor_utils::cloud2msg(debug_map_rejected, "camera_left");
    sensor_msgs::PointCloud2 pc2_curr_r = erasor_utils::cloud2msg(debug_curr_rejected, "camera_left");
...
void ERASOR::compare_vois_and_revert_ground_w_block(int frame)
{
    ...
    for (int theta = 0; theta < num_sectors; theta++)
    {
        for (int r = 0; r < num_rings; r++)
        {
            ...
            {
                r_pod_selected[r][theta].status = LITTLE_NUM; // 地图单元格为空
                continue;
            }

            if (bin_curr.points.size() < minimum_num_pts)
            {
                r_pod_selected[r][theta].status = LITTLE_NUM; // 查询帧单元格点数很少
            }
            else
            {
                // 计算扫描比例
                ...
                if (bin_curr.is_occupied && bin_map.is_occupied)
                {
                    ...
                    { // find dynamic!
                        ...
                        {                                                    // Occupied -> Disappear  <<BLUE>>
                            r_pod_selected[r][theta].status = MAP_IS_HIGHER; // 潜在动态区域：地图单元格高度差远大于查询单元格高度差
                        }
                        else if (map_h_diff <= curr_h_diff)
                        {                                                     // No objects exist -> Appear! <<GREEN>>
                            r_pod_selected[r][theta].status = CURR_IS_HIGHER; // 不存在动态区域：查询单元格高度差远大于地图单元格高度差，因为地图单元格点云已被擦出
                        }
                    }
                    else
                    {
                        r_pod_selected[r][theta].status = MERGE_BINS; // 不存在动态区域：地图单元格和查询帧单元格高度差近似
                    }
                }
                else if (bin_map.is_occupied)
                {                                                 // Maybe redundant?
                    r_pod_selected[r][theta].status = LITTLE_NUM; // 查询帧单元格不存在点
                }
            }
        }
        //    out_map<<"\n";  out_curr<<"\n";
            ...
            else if (OCCUPANCY_STATUS == MAP_IS_HIGHER)
            {
                if ((bin_map.max_h - bin_map.min_h) > 0.5)
                {
                    ...
                    *tmp = r_pod_selected[r][theta].points;
                    ...
                    ground_viz += piecewise_ground_;
                    debug_map_rejected += non_ground_; // 地图中的动态点
...
/**
 * @brief 检查单元格附近是否存在动态单元格
 *
 * @param r_pod_selected
 * @param r_target
 * @param theta_target
 * @param r_range
 * @param theta_range
...
bool ERASOR::is_dynamic_obj_close(R_POD &r_pod_selected, int r_target, int theta_target, int r_range, int theta_range)
{
    ...
    for (int j = theta_target - theta_range; j <= theta_target + theta_range; j++)
    {
        if (j < 0)
        {
            ...
            theta_candidates.push_back(j + num_sectors);
        }
...
/**
 * @brief 发布区域内估计的静态点和地面点
 *
 * @param arranged 区域内估计的静态点
 * @param complement 区域外的互补点
 */
void ERASOR::get_static_estimate(pcl::PointCloud<pcl::PointXYZI> &arranged, pcl::PointCloud<pcl::PointXYZI> &complement)
{
    ...
    {
        sensor_msgs::PointCloud2 pc2_ground = erasor_utils::cloud2msg(ground_viz, "camera_left");
        pub_ground.publish(pc2_ground);
    }

    // 发布估计静态点
    complement = map_complement;
    sensor_msgs::PointCloud2 pc2_arranged = erasor_utils::cloud2msg(arranged, "camera_left");
    sensor_msgs::PointCloud2 pc2_map_complement = erasor_utils::cloud2msg(complement, "camera_left");
